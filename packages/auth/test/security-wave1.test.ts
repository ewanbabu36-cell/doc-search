...
describe('Wave 1 Healthcare Security Foundation — Test Suite', () => {
  ...
  it('TEST 6: Missing exp is rejected (fail closed)', () => {
  ...
  it('TEST 15: Refresh token reuse detection revokes entire session family', async () => {
    ...
    // Session service should revoke the active session family
    ...
    // We update another session with old token hash to test reuse detection
    await assert.rejects(
      async () => {
        await sessionService.rotateRefreshToken({
