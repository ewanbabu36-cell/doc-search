...
import type { BiomedicalAuditTraceDto } from '@docsearch/api-contracts';

interface Props {
  traces: BiomedicalAuditTraceDto[];
}

export const AssetAuditVaultView: React.FC<Props> = ({ traces }) => {
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-bold text-gray-900">Biomedical Cryptographic Audit Trail & Integrity Vault</h2>
        <p className="text-xs text-gray-500">Immutable append-only ledger recording all equipment actions with SHA-256 signatures</p>
      </div>
