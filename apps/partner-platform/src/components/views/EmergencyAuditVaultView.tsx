...
import type { EmergencyAuditTraceDto } from '@docsearch/api-contracts';

interface Props {
  traces: EmergencyAuditTraceDto[];
}

export const EmergencyAuditVaultView: React.FC<Props> = ({ traces }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Emergency Immutable Audit Vault</h1>
        <p className="text-sm text-gray-500">Cryptographically signed, append-only ledger for all emergency triage decisions, code blue events, and trauma activations</p>
      </div>

      <Card className="p-4">
        <div className="mb-4">
          <Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search action, actor, trace ID or emergency code..." />
        </div>
        <Table>
          <thead>
          ...
          <tbody className="divide-y text-xs font-mono">
            {filtered.map((t) => (
              <tr key={t.id}>
                <td className="py-2 font-bold text-red-700">{t.traceNumber}</td>
